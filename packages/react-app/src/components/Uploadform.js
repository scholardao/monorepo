...
Web3Storage1 = async (input)=>{
  let newClient =  new Web3Storage({token : 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJkaWQ6ZXRocjoweGYwNTE5NUFBOWU3MUQzMmUxNDRGODhEM2MyYjVlNTVENDg2YzRhNEIiLCJpc3MiOiJ3ZWIzLXN0b3JhZ2UiLCJpYXQiOjE2Mjg0MzEwNDA3NzksIm5hbWUiOiJTaGVldGFsQ29kZXMifQ.c-V4d1lQycfu6hyjRNyzSD50xZWoTtUF_qgTg1j2H_s'})
  ...
  if(this.state.step == 4 && this.state.isSubmitted == false){
    ...
    this.Web3Storage1(this.state.uploadedPaper).then((cid)=>{
        holdValues.IPFSlink = cid
         ...
         console.log(`Subject:${subject} + subSubjects:${subSubjects} + titleString:${titleString} + IPFSlink:${IPFSlink} + papersCited:${papersCited} + validatorTip:${validatorTip} + validatorAddresses:${validatorAddresses} `)
         this.setState({isSubmitted: true})
         this.state.daiContract.submitDraft(
           titleString,
           IPFSlink,
           subject,
           subSubjects,
...
handleAuthorsChange = input=>e=>{
 ...
 render(){
    ...
    const {isSubmitted, step, title,authors1,authors2,authors3,authors4,authors5,IPFSlink,subject,subSubjects,papersCited,validatorTip,validatorAddresses,uploadedPaper } = this.state;
    const values = {title,IPFSlink,authors1,authors2,authors3,authors4,authors5,subject,subSubjects,papersCited,validatorTip,validatorAddresses,uploadedPaper  };
